...
public class ConsoleService {
     ...
     */
    public TenmoAccount selectFromUserList(List<TenmoAccount> accountList, boolean isSendNotRequest) {
        TenmoAccount selectedUserAccount = null;
        if (isSendNotRequest) {
            System.out.println("\nSelect user id to receive transfer: ");
        } else {
            System.out.println("\nSelect user id to request transfer: ");
        }
        for(int i = 0; i < accountList.size(); i++) {
            User[] usersForAccount =theTenmoService.getUserForAccount(accountList.get(i));
     ...
     */
    public TenmoTransfer selectFromTransferList(List<TenmoTransfer> userTransfers, String transferType) {
        TenmoTransfer selectedTransfer = null;
        System.out.println("\nSelect item number for details on transfer (or 0 to exit): \n");
            if (transferType.equals("send")) {
                System.out.println(" Transfer");
                System.out.println("   Id    \tSend From\tSend To\t\t Amount");
                System.out.println(" --------\t---------\t-------\t\t-------");
            } else {
                System.out.println(" Transfer");
                System.out.println("   Id    \tRequest By\tRequest To\t\t Amount");
                System.out.println(" --------\t----------\t----------\t\t-------");
            }
        for (int i = 0; i < userTransfers.size(); i++) {
            User[] toUser = theTenmoService.getUserForAccount(userTransfers.get(i).getToTenmoAccount());
            User[] fromUser = theTenmoService.getUserForAccount(userTransfers.get(i).getFromTenmoAccount());
            System.out.printf("%d. %d \t%-10s\t%-10s\t$%-10.2f\n", i + 1
    ...
    public void printTransferDetails(TenmoTransfer aTransfer) {
        ...
        if (aTransfer.getTransferType().equals(TenmoTransfer.TRANSFER_TYPE.REQUEST)){
            System.out.printf("Request From:\t%s (%d)\n", fromUser[0].getUsername(), aTransfer.getFromTenmoAccount().getUser_id());
            System.out.printf("  Request To:\t%s (%d)\n", toUser[0].getUsername(), aTransfer.getToTenmoAccount().getUser_id());
        } else {
     ...
     */
    public void displayAnAccount(TenmoAccount anAccount, AuthenticatedUser theUser) {
     ...
     */
    public void printTransfersSummary(List<TenmoTransfer> userTransfers) {
        for(TenmoTransfer aTransfer : userTransfers) {
